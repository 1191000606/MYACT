        ...
        while True and not rospy.is_shutdown():
            ...
            while t < max_publish_step and not rospy.is_shutdown():
                ...
                if config["policy_class"] == "ACT":
                    if t >= max_t:
                        pre_action = action
                        inference_thread = threading.Thread(target=inference_process, args=(args, config, ros_operator, policy, stats, t, pre_action))
                        inference_thread.start()
                        inference_thread.join()
                        # inference_process(args, config, ros_operator,policy, stats, t, pre_action)
                        inference_lock.acquire()
...
class RosOperator:
    ...
    def get_frame(self):
        if (
            ...
            or (self.args.use_depth_image and (len(self.img_left_depth_deque) == 0 or len(self.img_right_depth_deque) == 0 or len(self.img_front_depth_deque) == 0))
        ):
            return False
...
def get_arguments():
    ...
    parser.add_argument("--dilation", action="store_true", help="If true, we replace stride with dilation in the last convolutional block (DC5)", required=False)
    parser.add_argument(
        "--position_embedding", default="sine", type=str, choices=("sine", "learned"), help="Type of positional embedding to use on top of the image features", required=False
    )
    ...
    parser.add_argument("--puppet_arm_left_cmd_topic", action="store", type=str, help="puppet_arm_left_cmd_topic", default="/master/joint_left", required=False)
    parser.add_argument("--puppet_arm_right_cmd_topic", action="store", type=str, help="puppet_arm_right_cmd_topic", default="/master/joint_right", required=False)
    ...
    parser.add_argument("--robot_base_cmd_topic", action="store", type=str, help="robot_base_topic", default="/cmd_vel", required=False)
    parser.add_argument("--use_robot_base", action="store", type=bool, help="use_robot_base", default=False, required=False)
    parser.add_argument("--publish_rate", action="store", type=int, help="publish_rate", default=40, required=False)
    ...
    parser.add_argument("--arm_steps_length", action="store", type=float, help="arm_steps_length", default=[0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.2], required=False)
...
def main():
    ...
    json.dump(config, open("args.json", "w"), indent=4)
