...
def main(args):
    ...
    puppet_arm_left_publisher = rospy.Publisher(args.puppet_arm_left_topic, PoseStamped, queue_size=10)
    puppet_arm_right_publisher = rospy.Publisher(args.puppet_arm_right_topic, PoseStamped, queue_size=10)
    ...
    if args.only_pub_master:
        ...
        while not rospy.is_shutdown() and i < len(actions):
            ...
            left_pos = qposs[i][:3]
            left_ori = qposs[i][3:6]
            left_gripper = qposs[i][6]
            right_pos = qposs[i][7:10]
            right_ori = qposs[i][10:13]
            right_gripper = qposs[i][13]

            quat = R.from_euler("xyz", left_ori).as_quat()
            pose_msg.pose.orientation.x = quat[0]
            pose_msg.pose.orientation.y = quat[1]
            pose_msg.pose.orientation.z = quat[2]
            pose_msg.pose.orientation.w = quat[3]
            pose_msg.pose.position.x = left_pos[0]
            pose_msg.pose.position.y = left_pos[1]
            pose_msg.pose.position.z = left_pos[2]
            puppet_arm_left_publisher.publish(pose_msg)

            quat = R.from_euler("xyz", right_ori).as_quat()
            pose_msg.pose.orientation.x = quat[0]
            pose_msg.pose.orientation.y = quat[1]
            pose_msg.pose.orientation.z = quat[2]
            pose_msg.pose.orientation.w = quat[3]
            pose_msg.pose.position.x = right_pos[0]
            pose_msg.pose.position.y = right_pos[1]
            pose_msg.pose.position.z = right_pos[2]
            puppet_arm_right_publisher.publish(pose_msg)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--camera_names", action="store", type=str, help="camera_names", default=["cam_high", "cam_left_wrist", "cam_right_wrist"], required=False)
    ...
    parser.add_argument("--master_arm_left_topic", action="store", type=str, help="master_arm_left_topic", default="/master/joint_left", required=False)
    parser.add_argument("--master_arm_right_topic", action="store", type=str, help="master_arm_right_topic", default="/master/joint_right", required=False)

    parser.add_argument("--puppet_arm_left_topic", action="store", type=str, help="puppet_arm_left_topic", default="/puppet/joint_left", required=False)
    parser.add_argument("--puppet_arm_right_topic", action="store", type=str, help="puppet_arm_right_topic", default="/puppet/joint_right", required=False)

    parser.add_argument("--robot_base_topic", action="store", type=str, help="robot_base_topic", default="/cmd_vel", required=False)
    parser.add_argument("--use_robot_base", action="store", type=bool, help="use_robot_base", default=True, required=False)
